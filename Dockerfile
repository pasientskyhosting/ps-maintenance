...
COPY --from=pasientskyhosting/ps-worker:mono4.8.1 /etc/supervisord.conf /etc/supervisord.conf
...
COPY --from=pasientskyhosting/ps-worker:mono4.8.1 /start.sh /start.sh
